...
io.on('connection', (socket) => {
    socket.on('createRoom', ({ customDeck }) => {
        ...
        games[roomId] = { 
            ...
            board: null, 
            turn: socket.id, activePiece: null, 
            hasMoved: false, hasAttacked: false,
            rushMoveLeft: 2, rushAttackLeft: 1,
            berserkerBonusMove: false 
        };
        socket.join(roomId);
    ...
    socket.on('joinRoom', ({ roomId, customDeck }) => {
        ...
        const playerADeck = game.decks[game.players[0]];
        const playerBDeck = game.decks[game.players[1]];
        game.board = createInitialBoard(playerADeck, playerBDeck);
    ...
    socket.on('actionMove', ({ roomId, from, to }) => {
        ...
        if (piece.type === 'RushWarrior') {
            ...
            if (game.rushMoveLeft <= 0 && game.rushAttackLeft <= 0) { game.hasMoved = true; game.hasAttacked = true; }
        } 
        ...
        io.to(roomId).emit('actionProcessed', { board: game.board, hasMoved: game.hasMoved, hasAttacked: game.hasAttacked, activePiece: game.activePiece });
    ...
    socket.on('actionAttack', ({ roomId, from, to }) => {
        ...
        if (target.player !== attacker.player) {
            ...
            if (target.type === 'SuicideBomber') {
                game.board[to.y][to.x] = { type: 'Bomb', player: target.player, id: `Bomb-${Math.random()}` };
            } else {
                game.board[to.y][to.x] = null; 
            }
        }
        ...
        if (game.hasMoved && game.hasAttacked && !game.berserkerBonusMove) {
    ...
    function forceNextTurn(roomId) {
        ...
        for (let y = 0; y < 8; y++) {
            for (let x = 0; x < 8; x++) {
                ...
                if (p && p.type === 'Bomb' && p.player === nextTurnPlayer) {
                    game.board[y][x] = null; 
                }
            }
        }
